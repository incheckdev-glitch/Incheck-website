...
<section className="content-section detector-monitoring-section"><div className="shell content-grid detector-monitoring-grid">
  <Reveal><div className="detector-monitoring-copy"><span className="eyebrow">WHY CONTINUOUS MONITORING</span><h2>A manual reading shows one moment. Monitoring shows what happened between moments.</h2><p>Manual checks capture individual moments and remain important for process and context. Smart Detectors add the missing timeline—showing whether temperature or humidity stayed within the expected range between scheduled checks.</p><div className="bullet-grid">{['24/7 environmental visibility','Temperature monitoring','Humidity monitoring where applicable','Threshold alerts','Historical trends','Multi-location monitoring'].map(x=><div className="bullet-item" key={x}><span>✓</span>{x}</div>)}</div></div></Reveal>
  <Reveal delay={90}><div className="detector-monitoring-visual"><img src="/images/detectors/incheck360-monitoring-compare-visual.webp" alt="Freezer temperature history showing normal manual readings while continuous monitoring detects temperature excursions between checks." /></div></Reveal>
</div></section>
